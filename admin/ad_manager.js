...
/**************************************************************/
// fb_readAll(_path, _save, _func)
// Reads data from Firebase at a given path
// Calls _func with 'OK' status if data exists, otherwise logs an error
/**************************************************************/
function fb_readAll(_path, _save, _func){
  firebase.database().ref(_path).once('value', checkRead, fb_error)

  function checkRead(snapshot) {
      if(snapshot.val() == null) {
          console.error("errorrrrrr")
         
      } else {
          _func('OK', _path, snapshot);
      }
  }
}
...
/**************************************************************/
// fb_error(error)
// Handles Firebase errors by logging them to the console
/**************************************************************/
...
function ad_user() {
  ...
  fb_readAll(DETAILS, null, ad_processUSERReadAll);
}
...
function ad_processUSERReadAll(_result, _path, _snapshot, _save, _error) {
  console.log('%c ad_processUSERReadAll',
              'color: ' + COLAD_C + '; background-color: ' + COLAD_B + ';');
  console.log("Snapshot Value:", _snapshot.val());
  ...
  for (let uid in _snapshot.val()) {
      const userData = _snapshot.val()[uid];
      if (userData.Info) {
          ad_adminArray.push({
              displayName: userData.Info["Google Name"] || "No Name",
              email: userData.Info.Email || "No Email",
              name: userData.Info.Name || "No Name",
              age: userData.Info.Age || "N/A",
              uid: uid
          });
      }
  }

  ad_displayAll("t_userData", ad_adminArray, true, "", "", "", 4, DETAILS);
}*/
// Keep DETAILS as it is
DETAILS = '/auth/users/';

// 🔥 Updated: Build full field paths for Info
function ad_processUSERReadAll(_result, _path, _snapshot, _save, _error) {
  console.log('%c ad_processUSERReadAll',
              'color: ' + COLAD_C + '; background-color: ' + COLAD_B + ';');
  console.log("Snapshot Value:", _snapshot.val());
  ...
  ad_displayAll("t_userData", ad_adminArray, true, "", "", "", 5, DETAILS);
}
  ...
  function finishTdEdit(td, isOk, _path, _dbKey, _dbFieldName) {
    ...
    function finishTdEdit(td, isOk, _path, _dbKey, _dbFieldName) {
      if (isOk) {
        ...
        const trueFieldName = fieldMap[_dbFieldName] || _dbFieldName;
    
        console.log('%c finishTdEdit(): path/key = ' + _path + '/' +
          _dbKey + ',  field name = ' + trueFieldName +
          ', data = ' + td.innerHTML,
          'color: ' + COLAD_C + '; background-color: ' + COLAD_B + ';');
    
        var data = {};
        var rtn = [];
        rtn = ad_userInput(trueFieldName, td.innerHTML);
        if (rtn[0]) {        // User input validated ok?
          data[trueFieldName] = rtn[1];
          console.log('%c finishTdEdit(): td.innerHTML = ' + rtn[1] +
                      '  type = ' + typeof (rtn[1]),
                      'color: ' + COLAD_C + '; background-color: ' + COLAD_B + ';');
    
          td.style.background = 'red';
          ad_dbUpdateRec(_path, _dbKey, trueFieldName, data, td);
        }
        else {               // User input failed validation
          td.style.background = 'red';
          console.error('finishTdEdit(): validation failed for ' + rtn[1] +
...
function ad_dbUpdateRec(_path, _dbKey, _dbFieldName, _data, _td) {
  console.log('%c ad_dbUpdateRec(): _path/_dbKey = ' + _path + '/' +
              _dbKey + ',  _dbFieldName = ' + _dbFieldName +
              ',  _data = ' + JSON.stringify(_data) + '  _td = ' + _td,
              'color: ' + COLAD_C + '; background-color: ' + COLAD_B + ';');

  var dbRef = firebase.database().ref(_path + '/' + _dbKey + '/Info');
  dbRef.update(_data).then(function() {
    _td.style.background = 'Azure';
    console.log('%c ad_dbUpdateRec(): Update succeeded for ' +
                _path + '/' + _dbKey + '/Info',
